...
from twisted.conch import avatar, stdio
...
SERVER_RSA_PRIVATE = os.path.join(os.path.dirname(__file__), 'ssh-keys', 'ssh_host_rsa_key')
SERVER_RSA_PUBLIC = os.path.join(os.path.dirname(__file__), 'ssh-keys', 'ssh_host_rsa_key.pub')

# Path to RSA SSH keys accepted by the server.
CLIENT_RSA_PUBLIC = os.path.join(os.path.dirname(__file__), 'ssh-keys', 'client_rsa.pub')
...
class ExampleSession(object):
    ...
    def openShell(self, transport):
        ...
        """
        protocol = ServerProtocol(ConsoleManhole)
...
class TwistedSSHServer(factory.SSHFactory):
    """
