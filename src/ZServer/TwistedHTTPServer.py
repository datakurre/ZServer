...
class EchoServerProtocol(WebSocketServerProtocol):
    ...
    def onConnect(self, request):
        ...
        zf = ZmqFactory()
        e = ZmqEndpoint(
            ZmqEndpointType.connect,
            os.path.join(  # XXX: Currently unix only
                'ipc://' + config.instancehome,
                'zmq.sock',
                ).encode('utf-8'),
        )
        s = ZmqSubConnection(zf, e)
        s.subscribe(b"")
        s.gotMessage = lambda *args: self.onMessage(args[0], False)
        self.zmq = s

    def _connectionLost(self, reason):
        if self.zmq is not None:
            self.zmq.unsubscribe(b"")
        super(EchoServerProtocol, self)._connectionLost(reason)
...
def publish(message):
    global pub
    pub.publish(message.encode('utf-8'), b'')
...
class TwistedHTTPServer:
    ...
    def __init__(self, reactor, threadpool, publish_module):
        ...
        self.ws = WebSocketResource(factory)

    def render(self, request):
        if request.getHeader('upgrade') == 'websocket':
            return self.ws.render(request)
        else:
