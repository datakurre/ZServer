...
class BaseICPServer(asyncore.dispatcher):
    ...
    def clean_shutdown_control(self, phase, time_in_this_phase):
        if phase == 1:
            ...
            if time_in_this_phase < 2.0:
                # We have not yet been deaf long enough for our
                # front end proxies to notice.
    ...
    def calc_reply(self, request):
        if len(request) > 20:
            opcode, version, length, number, options, opdata, junk = \
                struct.unpack('!BBHIIII', request[:20])
            ...
            if version == 2:
                if opcode == ICP_OP_QUERY:
                    ...
                    return struct.pack(
                        '!BBHIIII', out_opcode, 2, 20, number, 0, 0, 0)
...
class ICPServer(BaseICPServer):
    # Products that want to do special ICP handling should .append their
    # hooks into this list. Each hook is called in turn with the URL as
    # a parameter, and they must return an ICP_OP code from above or None.
    # The first non-None return is used as the ICP response
    hooks = []
