...
class ftp_channel(asynchat.async_chat):
    ...
    read_only = 1
    write_commands = ['appe', 'dele', 'mkd', 'rmd', 'rnfr',
                      'rnto', 'stor', 'stou']
        ...
        if len(line) < 2:
            ...
            if not self.filesystem.isfile(file):
                ...
                try:
                    ...
                    self.respond('553 could not open file for reading: %s' % (
                        repr(why)))
                    return
                self.respond(
    ...
    def cmd_pass(self, line):
        ...
        if len(line) < 2:
            ...
            pw = line[1]
        result, message, fs = self.server.authorizer.authorize(
            self, self.user, pw)
        if result:
    ...
    def cmd_rest(self, line):
        ...
        self.respond(
            '350 Restarting at %d. Send STORE or '
            'RETRIEVE to initiate transfer.' % pos
        )
    ...
    def cmd_help(self, line):
        ...
        for attr in attrs:
            if attr[:4] == 'cmd_':
                x = getattr(self, attr)
                if isinstance(x, type(self.cmd_help)):
                    if x.__doc__:
...
class ftp_server(asyncore.dispatcher):
    ...
    def status(self):
        ...
        return producers.lines_producer([
            ...
            '<b>Current:</b> %d' % (self.total_sessions.as_long() -
                                    self.closed_sessions.as_long()),
            '<br>Files',
...
if os.name == 'posix':
    def test(port='8021'):
        ...
        try:
            ...
            fs.log_info(
                'FTP server shutting down. (received SIGINT)', 'warning')
