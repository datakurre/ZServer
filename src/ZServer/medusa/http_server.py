...
from ZServer.Zope2.Startup.config import (  # NOQA
    TRUSTED_PROXIES as trusted_proxies,
)
...
class http_request(object):
    ...
    def done(self):
        ...
        if self.version == '1.0':
            ...
            # to not want to type a version number
...
class http_channel(asynchat.async_chat):
    ...
    def found_terminator(self):
        if self.current_request:
            ...
            # ajung: we unquote() the request *after* calling crack_request
            # because this function breaks when it gets an unquoted request
            if '%' in request:
    ...
    def status(self):
        ...
        if self.total_clients:
            ratio = (self.total_requests.as_long() /
                     float(self.total_clients.as_long()))
        else:
            ratio = 0.0
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        sh = status_extension([hs, ms, ftp, cs, rs])
